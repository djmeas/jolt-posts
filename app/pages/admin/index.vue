...
async function loadPosts() {
  const { items } = await $fetch<{ items: any[]; hasMore: boolean }>('/api/posts')
  posts.value = items
}

async function loadConfig() {
  const config = await $fetch<{ displayName: string; sitePassword?: string; siteName: string }>('/api/config')
...
async function publishPost() {
  ...
  try {
    ...
    const res = await $fetch<{ photoPath: string }>('/api/admin/upload', { method: 'POST', body: fd })
    await $fetch('/api/posts', {
      method: 'POST',
      body: { photoPath: res.photoPath, description: description.value }
    })
    clearSelection()
    await loadPosts()
  } finally {
...
async function saveEdit() {
  ...
  const body: { description: string; createdAt: string; photoPath?: string } = {
  ...
  if (editSelectedFile.value) {
    ...
    try {
      const fd = new FormData()
      fd.append('file', editSelectedFile.value)
      const res = await $fetch<{ photoPath: string }>('/api/admin/upload', { method: 'POST', body: fd })
      body.photoPath = res.photoPath
    } finally {
      uploading.value = false
    }
  }
...
<template>
  <div :class="isDark ? 'min-h-screen bg-gradient-to-br from-[#0a0a0a] via-[#121212] to-[#0a0a0a] text-white' : 'min-h-screen bg-gradient-to-br from-gray-100 via-white to-gray-100 text-gray-900'">
    <input ref="editFileInput" type="file" accept="image/*" class="hidden" @change="handleEditFileSelect" />
    ...
    <main class="max-w-5xl mx-auto px-6 py-10">
      <div class="mb-12">
        ...
        <div v-if="!selectedFile">
          ...
          >
            ...
            <div v-else class="relative space-y-4">
              ...
              <div>
                <p :class="isDark ? 'text-gray-300 mb-1' : 'text-gray-700 mb-1'">Drag & drop your photo here</p>
        ...
        <div v-else class="space-y-4">
          <div class="relative rounded-2xl overflow-hidden bg-gradient-to-br from-gray-900 to-gray-800 p-1">
            <div class="relative rounded-xl overflow-hidden">
              <img :src="previewUrl!" class="w-full max-h-[500px] object-contain bg-black" />
              <button
                @click="clearSelection"
                class="absolute top-3 right-3 w-10 h-10 bg-black/60 backdrop-blur-sm rounded-full flex items-center justify-center text-white hover:bg-black/80 hover:scale-110 transition-all duration-200"
              >
        ...
        <div class="grid grid-cols-3 gap-3">
          ...
          >
            <div :class="isDark ? 'absolute inset-0 bg-gradient-to-br from-gray-800 to-gray-900 rounded-xl overflow-hidden' : 'absolute inset-0 bg-gradient-to-br from-gray-200 to-gray-300 rounded-xl overflow-hidden'">
              <img :src="post.photoPath" class="w-full h-full object-cover" />
    ...
    <Teleport to="body">
      ...
      >
        <div :class="isDark ? 'w-full max-w-md bg-gradient-to-br from-gray-900 to-black border border-white/10 rounded-2xl p-6 shadow-2xl' : 'w-full max-w-md bg-gradient-to-br from-white to-gray-100 border border-black/10 rounded-2xl p-6 shadow-2xl'">
          ...
          <div class="space-y-4">
            <div class="relative rounded-xl overflow-hidden bg-black">
              <img 
                :src="editPreviewUrl || editingPost?.photoPath" 
                class="w-full max-h-48 object-contain" 
              />
              <button
                @click="editFileInput?.click()"
                class="absolute bottom-2 right-2 px-3 py-1.5 bg-black/60 backdrop-blur-sm text-white text-xs font-medium rounded-lg hover:bg-black/80 transition-colors"
              >
                Change Photo
              </button>
            </div>
            ...
            <div class="flex gap-3 pt-2">
              <button
                ...
                :class="isDark ? 'text-white' : 'text-gray-900 bg-gray-200'"
              >
                Cancel
              </button>
              <button
